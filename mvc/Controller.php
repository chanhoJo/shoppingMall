...
abstract class Controller {
    ...
    public function __construct($application){
        ...
        $this->_viewMessage   = $application->getViewMessageObject();
    }
    ...
    protected function render(
        $param = array(), $viewFile = null, $template
        ...
        // view 클래스 인스탄스화
        $view = new View($this->_application->getViewDirectory(),
            // AppBase 클래스의 메소드
            // 뷰파일이 저장되어있는 폴더의 경로 반환
            $info // 전달정보 (리퀘스트,리퀘스트의 Base URL, Session)
        );
    ...
    protected function getToken($form){
        ...
        $tokens = $this->_session->get($key, array()); // $_SESSION변수에 Token을 저장하기 위한 키 이름 작성
